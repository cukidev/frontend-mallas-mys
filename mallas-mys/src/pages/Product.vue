...
const route=useRoute(); const router=useRouter(); const slug=route.params.slug as string;
const product = computed(()=> products.find(p=>p.slug===slug)); if(!product.value){ router.replace("/catalogo"); }
const imagesMap = import.meta.glob("/src/assets/products/*", { eager:true, query:"?url", import:"default" }) as Record<string,string>;
const activeImg = ref(product.value ? `/src/assets/products/${product.value.images[0]}` : ""); const chosenSize=ref<string>(""); const chosenColor=ref<string>("");
...
function addFav(){ if(product.value) toggleFavorite(product.value.id); }
function waHref(){ const name=product.value?.name ?? ""; const t=encodeURIComponent(`Hola, quiero cotizar ${name} talla ${chosenSize.value || "(por definir)"} color ${chosenColor.value || "(por definir)"}.`); return `https://wa.me/${WA_NUMBER}?text=${t}`; }
</script>
    ...
    <div class="product-grid">
      <div class="gallery">
        ...
        <div class="gallery-thumbs">
          <button v-for="img in product.images" :key="img" class="thumb" @click="activeImg = `/src/assets/products/${img}`">
            <img :src="imagesMap[`/src/assets/products/${img}`]" :alt="product.name" class="w-full h-full object-cover"/>
          </button>
        </div>
      </div>
      ...
      <div>
        ...
        <p class="info-text">Diseño confeccionado para patinaje artístico. Personaliza color y talla según tu necesidad.</p>
        ...
        <div class="actions">
          ...
          <a :href="waHref()" target="_blank" rel="noopener" class="btn-primary">Cotizar por WhatsApp</a>
